...
def _organize(flat, span_only=True, answered_only=True):
    ...
    for qid, passages, query, answers in flat:
        ...
        answers_words = []
        for ans in answers:
            answers_words += word_tokenize(ans)
        answers_words_no_stopwords = []
        for word in answers_words:
            if word not in stopword_word2id:
                answers_words_no_stopwords.append(word)

        for ind, passage in enumerate(passages):
            passage_text = passage['passage_text'].lower()
            #answer_vector = np.zeros(len(passage_text))

            if passage.get('is_selected', False) == False:
                if answered_only:
                    continue
                organized.append((qid, passage_text, query,
                                  [], False))
            if passage.get('is_selected', False) == True:
                organized.append((qid, passage_text, query,
                                  answers_words_no_stopwords, True))
...
def read_msmarco_examples(input_file, is_training):
    ...
    if is_training:
        ...
        organized, filtered_out = _organize(flat, answered_only=True, span_only=False)
    ...
    for qid, passage_text, query, answer_words, is_select in organized:
        #print(is_select)
        example = SquadExample(
            qas_id=qid,
            question_text=query,
            doc_tokens=passage_text,
...
def convert_examples_to_features(example, tokenizer, max_seq_length,
    ...
    query_tokens = word_tokenize(example.question_text)
    ...
    orig_to_tok_index = []
    all_doc_tokens = []
    example_doc_tokens = word_tokenize(example.doc_tokens)

    answer_vector = np.zeros(max_seq_length)
    if True:
        for i, token in enumerate(example_doc_tokens):
            if token in example.answer_words:
                answer_vector[len(query_tokens) + 2 + i] = 1.0
    max_tokens_for_doc = max_seq_length - len(query_tokens) - 3

    if len(example_doc_tokens) > max_tokens_for_doc:
        example_doc_tokens = example_doc_tokens[:max_tokens_for_doc]
        answer_vector = answer_vector[:max_tokens_for_doc]
    tokens = ["[CLS]"] + query_tokens + ["[SEP]"] + example_doc_tokens + ["[SEP]"]
...
def main():
    ...
    parser.add_argument("--do_find_words", default=True, action='store_true', help="Whether to find keywords.")
