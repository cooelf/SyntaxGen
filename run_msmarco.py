    ...
    for qid, passages, query, answers in flat:
        ...
        if not span_only or not answered_only:
            for ind, passage in enumerate(passages):
                if ind in matching:
                    continue
                if passage.get('is_selected', 0)==0:
                    matching.add(ind)
                    organized.append((qid, passage, query,
                                      (0, len(passage))))
                elif not answered_only:
                    #contain answer but do not match answer span
                    matching.add(ind)
                    ans_tokens = word_tokenize(answers[0])

                    p = word_tokenize(passage['passage_text'])
                    #print(passage)
                    #print(answers[0])
                    best_match_score = -1
                    best_fake_answer = ''
                    for start_tidx in range(len(p)):
                        if p[start_tidx] not in ans_tokens:
                            continue
                        for end_tidx in range(len(p) - 1, start_tidx - 1, -1):
                            span_tokens = p[start_tidx: end_tidx + 1]
                            if len(ans_tokens) > 0:
                    ...
                    p = ' '.join(p)
                    pos = p.find(best_fake_answer)


                    organized.append((qid, passage, query,
                                      (pos, pos+len(answers[0]))))
...
def main():
    ...
    parser.add_argument("--predict_file", default='MSMarco/data/dev_v2.1_top5.json', type=str,
