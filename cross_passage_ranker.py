...
from pytorch_pretrained_bert.modeling import BertForCrossPassagerRanker
...
def read_msmarco_examples(num, input_file, is_training):
    ...
    if True:
        with open(input_file, "r") as reader:
            ...
            flat = ((qid, passages[qid], queries[qid], answers.get(qid))
                    for qid in query_ids if qid in passages)

    if is_training:


        organized = _organize(flat, answered_only=False, span_only=False)
        organized = organized[:1000]
        ...
        organized = _organize(flat, span_only=False, answered_only=False)
        organized = organized[:1000]
    ...
    for qid, passage_id, passage, query, (start, stop), is_select in organized:
        #print(is_select)
        doc_tokens = []
        char_to_word_offset = []
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--train_file", default='MSMarco/data/dev_v2.1.json', type=str, help="MSMARCO json for training. E.g., train-v2.1.json")
    ...
    parser.add_argument("--warmup_proportion", default=0.3, type=float,
    ...
    # Prepare model
    model = BertForCrossPassagerRanker.from_pretrained(args.bert_model,
    ...
    if args.do_train:
        ...
        all_q_id = [f.unique_id for f in train_features]
        all_p_id = torch.tensor([f.example_index for f in train_features], dtype=torch.long)
        ...
        last_id = train_features[0].unique_id
        ...
        for i, f in enumerate(train_features):
            if f.unique_id == last_id:
                one_q_id.append(f.unique_id)
                one_p_id.append(f.example_index)
                one_input_ids.append(f.input_ids)
                one_input_mask.append(f.input_mask)
                one_segment_ids.append(f.segment_ids)
                one_is_select.append(f.is_select)
                if len(one_q_id)==10:
                    has = False
                    for select in one_is_select:
                        if select == 1:
                            has = True
                    if has:
                        all_p_id.append(one_p_id)
                        all_q_id.append(one_q_id)
                        all_input_ids.append(one_input_ids)
                        all_input_mask.append(one_input_mask)
                        all_segment_ids.append(one_segment_ids)
                        all_is_select.append(one_is_select)
                        one_q_id = []
                        one_p_id = []
                        one_input_ids = []
                        one_input_mask = []
        ...
        all_input_ids = all_input_ids.reshape(all_input_ids.size(0), 10*384)
        all_input_mask = torch.tensor(all_input_mask,  dtype=torch.long)
        all_input_mask = all_input_mask.reshape(all_input_mask.size(0), 10*384)
        all_segment_ids = torch.tensor(all_segment_ids, dtype=torch.long)
        all_segment_ids = all_input_mask.reshape(all_segment_ids.size(0), 10 * 384)
        all_is_select = torch.tensor(all_is_select, dtype=torch.long)


        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_is_select)
        if args.local_rank == -1:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", mininterval=1)):
                ...
                input_ids, input_mask, segment_ids, is_selects = batch
                input_ids = input_ids.reshape(-1, 384)
                input_mask = input_mask.reshape(-1, 384)
                segment_ids = segment_ids.reshape(-1, 384)
            ...
            output_model_file = os.path.join(args.output_dir, "pytorch_model.bin"+str(epoch))
            torch.save(model_to_save.state_dict(), output_model_file)

    # Load a trained model that you have fine-tuned
    output_model_file = os.path.join(args.output_dir, "pytorch_model.bin0")
    model_state_dict = torch.load(output_model_file)
    model = BertForCrossPassagerRanker.from_pretrained(args.bert_model, state_dict=model_state_dict)
    model = torch.nn.DataParallel(model)
    model.to(device)
    ...
    if args.do_fliter and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_p_id = torch.tensor([f.example_index for f in eval_features], dtype=torch.long)
