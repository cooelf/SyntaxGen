...
def read_squad_examples(input_file, is_training):
    ...
    for line in lines[:200]:
        ...
        if len(info) == 4:
            ...
            uid, title, context, query, answer, begin_answer, end_answer = line.split('\t')
            ...
            answer_offset = context.find(answer)
            #print(context)
            #print(answer)
            #print(answer_offset)
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    with open(output_prediction_file, "w") as writer:
        for k, v in all_predictions.items():
            writer.writelines(json.dumps({'query_id': int(k), 'answers': [v]}))
            writer.writelines('\n')
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--train_file", default='MSMarco/data/train.tsv', type=str,
                        help="SQuAD json for training. E.g., train-v1.1.json")
    parser.add_argument("--predict_file", default='MSMarco/data/dev.tsv', type=str,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", mininterval=1)):
                ...
                input_ids, input_mask, segment_ids, starts, ends = batch
                loss = model(input_ids, segment_ids, input_mask, starts, ends)
