...
def _organize(flat, training=True, answered_only=True):
    """
    ...
    for qid, passages, query, answers in flat:
        if answers == ['No Answer Present.'] and training:
            continue
        all_passages = ''
        ...
        for passage in passages:
            if passage['is_selected'] == True:
                is_select = True
            all_passages += passage['passage_text']
            all_passages += '\n'
        ...
        if answers is None and not answered_only and training:
        ...
        for ans in answers:
            if len(ans) == 0:
                continue
            pos = all_passages.find(ans)
            if not training:
                organized.append((qid, all_passages, query,
                                  (0, 3)))
                continue
...
def read_msmarco_examples(input_file, is_training):
    ...
    if is_training:
        organized, filtered_out = _organize(flat, training=True, span_only=False)
        print(len(organized))
        print(filtered_out)
        organized = organized
        ...
        organized, filtered_out = _organize(flat, training=False, answered_only=False)
        organized = organized
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--train_file", default='MSMarco/data/train_v2.1_top5.json', type=str, help="MSMARCO json for training. E.g., train-v2.1.json")
    parser.add_argument("--predict_file", default='MSMarco/data/dev_v2.1_cheat.json', type=str,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", mininterval=10)):
                if n_gpu == 1:
