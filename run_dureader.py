...
from pytorch_pretrained_bert.tokenization_dureader import whitespace_tokenize, BasicTokenizer, BertTokenizer
from pytorch_pretrained_bert.modeling_dureader import BertForQuestionAnswering
...
def paral_read(line, args, is_training):
    one_examlpes = []
    ...
    if is_training:
        if len(sample['answer_spans']) == 0:
            return one_examlpes
        if sample['answer_spans'][0][1] >= args.max_seq_length:
            return one_examlpes
    ...
    sample['passages'] = []
    question = seg_char(sample['question'])
    if len(sample['answers']) != 0:
        ans = seg_char(sample['answers'][0])
    else:
        return None
    for d_idx, doc in enumerate(sample['documents']):
        most_related_para = doc['most_related_para']

        doc_tokens = seg_char(doc['paragraphs'][most_related_para])
        if is_training == False:
            example = SquadExample(
                ...
                start_position=0,
                end_position=args.max_seq_length,
                is_select=doc['is_selected'])
            one_examlpes.append(example)
            continue
        ...
        start_tidx = 0
        end_tidx = 0
        for start_tidx in range(len(doc_tokens)):
            if doc_tokens[start_tidx] not in ans:
                continue

            for end_tidx in range(min(len(doc_tokens) - 1, args.max_seq_length), start_tidx - 1, -1):
                span_tokens = doc_tokens[start_tidx: end_tidx + 1]
                if len(ans) > 0:
        ...
        example = SquadExample(
            ...
            question_text=question,
            doc_tokens=doc_tokens,
            start_position=best_match_span[0],
            end_position=best_match_span[1],
            is_select=doc['is_selected'])
        one_examlpes.append(example)

    return one_examlpes

def read_dureader_examples(args, input_file, is_training):
    ...
    partial_read_dureader = partial(paral_read,
                                    args=args,
                                    is_training=is_training)
    results = pool.map(partial_read_dureader, lines)
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        question_tokens = example.question_text
        ...
        if is_training:
            ...
            start_position = example.start_position + question_len
            end_position = example.end_position + question_len
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for feature in all_features:
    example_index_to_features[feature.q_id].append(feature)
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    features = example_index_to_features[example.qas_id]
    if example.qas_id in all_nbest_json:
        continue

    prelim_predictions = []
    for (feature_index, feature) in enumerate(features):

      result = unique_id_to_result[str(feature.q_id) + '_' + str(feature.doc_id)]
    ...
    for pred in prelim_predictions:
      ...
      final_text = tok_text.replace('<splitter>', ',')
...
def main():
    ...
    parser.add_argument("--output_dir", default='dureader/record', type=str, required=False,
                        help="The output directory where the model checkpoints and predictions will be written.")

    ## Other parameters
    parser.add_argument("--train_file", default='dureader/data/search.train1.json', type=str,
                        help="SQuAD json for training. E.g., train-v1.1.json")
    parser.add_argument("--predict_file", default='dureader/data/search.dev.json', type=str,
    ...
    if args.do_train:
        train_examples_file = os.path.join('dureader', 'data', 'tran_examples')

        try:
            with open(train_examples_file, "rb") as reader:
                train_examples = pickle.load(reader)
        except:
            train_examples = read_dureader_examples(args,
                input_file=args.train_file, is_training=True)
            with open(train_examples_file, "wb") as reader:
                pickle.dump(train_examples, reader)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                all_is_selects = torch.tensor([f.is_select for f in train_features], dtype=torch.long)
                #all_answer_vectors = torch.tensor([f.answer_vector for f in train_features], dtype=torch.float)
                train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_positions,
                                           all_end_positions, all_is_selects)
                if args.local_rank == -1:
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", mininterval=1)):
                    ...
                    input_ids, input_mask, segment_ids, start, end, is_select = batch
                    loss = model(input_ids, segment_ids, input_mask, start, end, is_select)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = read_dureader_examples(args,
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for i, example_index in enumerate(example_indices):
                ...
                unique_id = str(eval_feature.q_id) + '_' + str(eval_feature.doc_id)
